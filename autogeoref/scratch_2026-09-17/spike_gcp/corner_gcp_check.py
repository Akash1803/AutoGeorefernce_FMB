...
def edge_matches(c, v, band=1.5, dth=math.radians(12)):
...
for f in sorted(glob.glob(os.path.join(bk,"*.gpkg"))):
    ...
    def span(idx):
        ...
        P=np.array([pts[i] for i in idx]); return max(np.linalg.norm(P[a]-P[b]) for a in range(len(P)) for b in range(a+1,len(P)))
